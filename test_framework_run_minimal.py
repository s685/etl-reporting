...
# Minimal parser for --help and required-args validation (avoids heavy imports)
def _parse_args():
...
if __name__ == '__main__':
    args = _parse_args()
    # Deferred imports and execution
    ...
    from datamart_analytics.definitions.custom_definitions import SourceTargetDatabaseConnectionType
    ...
    def load_test(
        source_database_name, target_database_name, database_warehouse, table_schema_name,
        specification_csv_path, source_target_database_connection_type, sql_server_host,
        sql_server_port, sql_to_snowflake_query_file_path
    ):
        try:
            ...
            if source_target_database_connection_type == SourceTargetDatabaseConnectionType.SNOWFLAKE_TO_SNOWFLAKE:
                if not os.path.exists(specification_csv_path):
                    raise CSVFileNotFoundException(f'Specification CSV file not found: {specification_csv_path}')
    ...
    try:
        conn_type = getattr(SourceTargetDatabaseConnectionType, args.source_target_database_connection_type)
    except AttributeError:
        conn_type = SourceTargetDatabaseConnectionType.SNOWFLAKE_TO_SNOWFLAKE
    try:
        load_test(args.source_database_name, args.target_database_name, args.database_warehouse,
                  args.table_schema_name, args.specification_csv_path, conn_type,
