...
- Duplicate primary key detection with TRIM
...
"""
    ...
    def _generate_row_count_test(
        self,
        table_name: str,
        filter_column: str,
        check_name: str
    ) -> str:
        """Generate total row count test for table."""
        sql = f"""-- Test: {table_name} (Total Row Count)
SELECT 
    '{check_name}' AS check_name,
    COUNT(*) AS total_rows
FROM {table_name}
...
"""
    ...
    def _generate_duplicate_pk_test(
        self,
        table_name: str,
        primary_key: str,
        filter_column: str,
        ...
        """Generate duplicate primary key test with TRIM."""
        # Split comma-separated primary keys
        pk_columns = [col.strip() for col in primary_key.split(',')]
        
        # Build trimmed column list for SELECT and GROUP BY
        trimmed_selects = []
        trimmed_group_by = []
        
        for i, col in enumerate(pk_columns):
            trimmed_selects.append(f"    LTRIM(RTRIM({col})) AS pk_col_{i+1}")
            trimmed_group_by.append(f"LTRIM(RTRIM({col}))")
        
        select_clause = ',\n'.join(trimmed_selects)
        group_by_clause = ', '.join(trimmed_group_by)
        
        sql = f"""-- Test: {table_name}.{primary_key} (Duplicate Primary Key Check with TRIM)
SELECT 
    '{check_name}' AS check_name,
{select_clause},
...
"""
    ...
    def generate_test_cases(
        ...
        test_types: List[str] = ['row_count', 'duplicate', 'count', 'sum']
    ) -> str:
        ...
        sql_output.append(f"-- Test Types: {', '.join(test_types)}")
        sql_output.append("-- =====================================================================")
        sql_output.append("")
        
        # Track tables we've already processed for table-level tests
        ...
        for table_name, column_list, primary_key, filter_column in mapping:
            ...
            # Generate table-level tests (once per table)
            if table_short not in processed_tables:
                processed_tables.add(table_short)
                
                # 1. Row Count Test
                if 'row_count' in test_types:
                    check_name = f"{table_short.upper()}_ROW_CNT"
                    test_sql = self._generate_row_count_test(
                        full_table, filter_column, check_name
                    )
                    sql_output.append(test_sql)
                
                # 2. Duplicate Primary Key Test
                if 'duplicate' in test_types and primary_key:
                    check_name = f"{table_short.upper()}_DUP_PK"
                    test_sql = self._generate_duplicate_pk_test(
                        full_table, primary_key, filter_column, check_name
                    )
                    sql_output.append(test_sql)
            
            # 3. Column-level tests
...
def parse_args():
    ...
    parser.add_argument(
        "--test-types",
        nargs='+',
        choices=['row_count', 'duplicate', 'count', 'sum'],
        default=['row_count', 'duplicate', 'count', 'sum'],
        help="Types of tests to generate (default: all)"
    )
...
def main():
    ...
    # Generate SQL
    sql_output = generator.generate_test_cases(mapping, args.test_types)
